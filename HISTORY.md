...
DeepBizには**2つのデータ収集アプローチ**があります：

#### 方式A: CSV Import（オプション - 今回採用）
```
CSV（企業名、住所のリスト）
  ↓
Salonテーブル登録（Place ID無し）
  ↓
企業名+住所でGoogle Maps検索 ← キーワード: "菊池皮膚科クリニック 東京都千代田区"
  ↓
Place ID取得・保存
  ↓
Phase 1エンリッチメント
...
**特徴**:
- ✅ **特定企業を狙い撃ち**（リスト化済みの企業を対象）
- ✅ **精度が高い**（企業名で検索するため誤検出が少ない）
- ❌ **網羅性は限定的**（CSV提供元のリスト範囲内のみ）

#### 方式B: タスクベース（標準 - 将来の多業種展開用）
```
タスク生成（エリア×業種）
  ↓
エリア×業種でGoogle Maps検索 ← キーワード: "東京都千代田区丸の内 美容クリニック"
  ↓
Place ID取得（最大20件/タスク）
  ↓
Salonテーブル登録（初めからPlace ID有り）
  ↓
Phase 1エンリッチメント
```

**特徴**:
- ✅ **網羅的な発見**（そのエリアの該当業種を全検索）
- ✅ **スケーラブル**（3,423タスク × 20件 = 最大68,460件発見可能）
- ❌ **ノイズ混入**（関連性低い店舗も含まれる可能性）

**共通点**: 両方式とも最終的には**Google Maps検索 → Place ID取得**という同じ処理に収束

**今回**: トリビュー社提供のCSVがあったため、方式Aを採用（1,905件のショートカット）
